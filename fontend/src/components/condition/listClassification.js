...
import { queryConditionState, conditionState } from "@/atom";
...
const StatusTable = () => {
  const [classification, setClassification] = useRecoilState(conditionState);

  const query = useRecoilValue(queryConditionState);
  ...
  const fetchProduct = async (query) => {
    try {
      const response = await axiosClient.get(`/v1/condition${query}`, {
        withCredentials: true,
      });
      if (response.data.success) {
        setClassification(response.data.condition);
      } else {
